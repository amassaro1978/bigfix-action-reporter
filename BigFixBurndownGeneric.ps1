...
.DESCRIPTION
    Enter multiple Action IDs from any BigFix deployment. The tool combines all successful
...
<Window
    ...
    Title="BigFix Deployment Burndown" Width="1100" Height="750"
    ...
    <Grid Margin="20">
        ...
        <!-- Title -->
        <TextBlock Grid.Row="0" Text="Deployment Burndown" FontSize="24" FontWeight="Light"
        ...
        <StackPanel Grid.Row="4" Orientation="Horizontal" Margin="0,0,0,12">
            <Border Background="#313244" CornerRadius="6" Padding="16,10" Margin="0,0,12,0">
                <StackPanel Orientation="Horizontal">
                    <TextBlock Text="Remaining:" Foreground="#a6adc8" FontFamily="Segoe UI" Margin="0,0,8,0"/>
                    <TextBlock x:Name="lblRemaining" Text="--" Foreground="#f38ba8" FontFamily="Segoe UI" FontWeight="Bold" FontSize="16"/>
                </StackPanel>
            </Border>
            <Border Background="#313244" CornerRadius="6" Padding="16,10" Margin="0,0,12,0">
                <StackPanel Orientation="Horizontal">
                    <TextBlock Text="Completed:" Foreground="#a6adc8" FontFamily="Segoe UI" Margin="0,0,8,0"/>
...
# Secondary Y axis for completed
...
# Burndown line (remaining) -- red, declining
$burnSeries = New-Object System.Windows.Forms.DataVisualization.Charting.Series "Remaining"
...
# Completed line -- green, rising
$installSeries = New-Object System.Windows.Forms.DataVisualization.Charting.Series "Completed"
...
$btnGenerate.Add_Click({
    try {
        ...
        # Update chart
        $chart.Series["Remaining"].Points.Clear()
        $chart.Series["BurndownFill"].Points.Clear()
        $chart.Series["Completed"].Points.Clear()
        ...
        if ($result.BurndownData.Count -gt 0) {
            ...
            $chart.Series["Remaining"].Points.AddXY($startLabel, $totalMachines) | Out-Null
            $chart.Series["BurndownFill"].Points.AddXY($startLabel, $totalMachines) | Out-Null
            $chart.Series["Completed"].Points.AddXY($startLabel, 0) | Out-Null
        ...
        foreach ($row in $result.BurndownData) {
            ...
            $chart.Series["Remaining"].Points.AddXY($label, $row.LegacyRemaining) | Out-Null
            $chart.Series["BurndownFill"].Points.AddXY($label, $row.LegacyRemaining) | Out-Null
            $chart.Series["Completed"].Points.AddXY($label, $row.TotalInstalled) | Out-Null
        ...
        $lblStatus.Text = "Burndown generated: $($result.UniqueInstalls)/$totalMachines completed (${pct}%) - Actions: $actionList"
...
$btnExport.Add_Click({
    ...
    $dlg.FileName = "BigFix_Burndown_$(Get-Date -Format 'yyyyMMdd').csv"
    if ($dlg.ShowDialog()) {
